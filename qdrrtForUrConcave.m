% rrt for ur10(3d)
% concave obstacles for reviewer 4 comment 5
function [time,pathLength,path] = qdrrtForUrConcave()
...
rec2 = [0.5,-1.5000,0.600;
    ...
    0.5,-0.5000,0.5];
rec2(:,2)=rec2(:,2)-0.3;
of(:,:,2) = [1,2,8,7;3,4,6,5;1,2,6,5;3,4,8,7;1,7,3,5;2,8,4,6];
for i = 1:6
    h = patch(rec2(of(i,:,2),1),rec2(of(i,:,2),2),rec2(of(i,:,2),3),'g');
    set(h,'facealpha',0.2);
end
rec3 = [0.5,-1.5000,-0.300;
    0.5,-1.5000,-0.4;
    -0.3,-0.5,-0.3;
    -0.3,-0.5,-0.4;
    -0.3,-1.5000,-0.3;
    ...
    0.5,-0.5000,-0.4];
rec3(:,2)=rec3(:,2)-0.3;
of(:,:,3) = [1,2,8,7;3,4,6,5;1,2,6,5;3,4,8,7;1,7,3,5;2,8,4,6];
for i = 1:6
...
rec4 = [0.6,-0.5,0.6;
    ...
    0.6,-1.5,-0.4];
rec4(:,2)=rec4(:,2)-0.3;
of(:,:,4) = [1,2,8,7;3,4,6,5;1,2,4,3;5,6,8,7;1,3,5,7;2,4,6,8];
for i = 1:6
    h = patch(rec4(of(i,:,4),1),rec4(of(i,:,4),2),rec4(of(i,:,4),3),'g')
    set(h,'facealpha',0.2);
end
obstacles(:,:,1) = rec1;
obstacles(:,:,2) = rec2;
...
point0 = [pi/2,0,0,0,0,0];
point1 = [145,30,-70,-50+360,0,0]*pi/180;
stepsize = 0.2;
disTh = 0.2;
